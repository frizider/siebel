...
class Auth extends CI_Controller {
	...
	function index()
	{
		...
		else
		{
			...
			//list the users
			$group = (isset($_POST['search_usergroup']) && !empty($_POST['search_usergroup'])) ? $_POST['search_usergroup'] : FALSE;
			...
			$this->load->view('DomainView', $this->data);
	...
	function login()
	{
		...
		else
		{
			...
			$this->data['identity'] = array('name' => 'identity',
				...
				'rel' => 'popover',
				'title' => 'For customers',
				'data-content' => 'Your username is equal to your customer number which is found on every bill.',
	...
	{
		$this->data['form_attributes'] = array('class' => 'form-horizontal');
		...
		if ($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email, $additional_data, $group, $emails))
		{
	...
	function edit_user($id)
	{
		...
		elseif ($this->ion_auth->logged_in() && $this->ion_auth->is_admin() || perm('Edit all accounts')) 
		{
			$user = $this->ion_auth->user($id)->row();
		}
		
		elseif ($this->ion_auth->logged_in() && perm('Edit own account') && $this->uri->segment(3) != user('id')) 
		{
			$id = user('id');
			$this->session->set_flashdata('message', 'You are only permitted to edit your own account.');
			redirect('auth/edit_user/'.$id, 'refresh');
		}

		elseif ($this->ion_auth->logged_in() && perm('Edit own account')) 
		{
			$id = user('id');
			$user = $this->ion_auth->user($id)->row();
		}
		...
		if (isset($_POST) && !empty($_POST))
		{
			...
			if ($this->form_validation->run() === TRUE)
			{
				$this->ion_auth->update($user->id, $data, $group, $emails);
		...
		$this->data['lang'] = array(
			...
			'class' => 'lang',
			'type' => 'hidden',
			'value' => $this->form_validation->set_value('lang', $user->lang)
		);
		$this->data['group'] = array(
			'name' => 'group',
			'id' => 'group',
	...
	function permissions()
	{
		...
		// Load the general view
		$this->data['view'] = 'auth/permissions';
		$this->load->view('DomainView', $this->data);
	}
	...
	function create_permission()
	{
		...
		$this->form_validation->set_rules('description', $this->siebel->getLang('description'), 'xss_clean');
		
		if ($this->form_validation->run() == TRUE)
		{
			if($this->permissions_model->create_permission($this->input->post('name'), $this->input->post('description'), $this->input->post('groups')))
			{
				// check to see if we are creating the permission
				// redirect them back to the admin page
				$this->session->set_flashdata('success', $this->ion_auth->messages());
				redirect("auth/permissions", 'refresh');
			}
		}
		else
		{
			...
			// Load the general view
			$this->data['view'] = 'auth/create_permission';
			$this->load->view('DomainView', $this->data);
		}
	}
	...
	function edit_permission()
	{
		...
		if (isset($_POST) && !empty($_POST))
		{
			if($this->permissions_model->update_permission($id, $_POST['name'], $_POST['description'], $_POST['groups']))
			{
			...
			{
				$this->session->set_flashdata('message', $this->ion_auth->errors());
				redirect("authpermissions/".$_POST['id'], 'refresh');
			}
			...
			$this->data['name'] = array(
				...
				'id'    => 'name',
				'type'  => 'text',
				'value' => $this->form_validation->set_value('name', $permission[0]['name']),
			);
			$this->data['description'] = array(
				'name'  => 'description',
				'id'    => 'description',
				'type'  => 'text',
				'value' => $this->form_validation->set_value('description', $permission[0]['description']),
			);
		...
		// Load the general view
		$this->data['view'] = 'auth/edit_permission';
		$this->load->view('DomainView', $this->data);
	}
