...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $additional_data = array(), $group = NULL, $emails)
	{
		...
		foreach($emails as $email) {
			$this->db->insert('email_users', array('user_id' => $id, 'state_id' => $email));
		};
	...
	public function users($groups = NULL)
	{
		...
		if(isset($_POST) && !empty($_POST)) {
			...
			if(isset($_POST['search_username']) && !empty($_POST['search_username'])) { $this->db->like('username', $_POST['search_username']); };
			if(isset($_POST['search_useremail']) && !empty($_POST['search_useremail'])) { $this->db->like('email', $_POST['search_useremail']); };
			if(isset($_POST['search_userlang']) && !empty($_POST['search_userlang'])) { $this->db->like('lang', $_POST['search_userlang']); };
	 ...
	 **/
	public function update($id, array $data, $group, $emails)
	{
		...
		$this->db->delete('users_groups', array('user_id' => $user->id));
		$this->db->insert('users_groups', array('user_id' => $user->id, 'group_id' => $group));

		$this->db->delete('email_users', array('user_id' => $user->id));
		
		foreach($emails as $email) {
			$this->db->insert('email_users', array('user_id' => $user->id, 'state_id' => $email));
		}
