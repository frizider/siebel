...
class Test_model extends CI_Model
{
	...
	public function getSheets() {
		$db = $this->load->database('packaging', TRUE);
		$db->where('deleted', 0);
		$db->where('method', 23);
		$db->where_in('finish', array(191, 192, 193));
		$sheets = $db->get('sheets')->result();
		//$return = $this->addSalesOrders($sheets);
		return $sheets;
	}
	...
	public function addSalesOrders($sheets) {
		
		foreach($sheets as $sheet) {
			$sheet->salesorders = $this->getOrderLines($sheet->profile, $sheet->customernumber);
			
			$tonnage_ordered = 0;
			$tonnage_delivered = 0;
			foreach ($sheet->salesorders as $order) {
				$tonnage_ordered += $order->ordertonnage;
				$tonnage_delivered += $order->deliveredtonnage;
			}
			$sheet->ordertonnage = $tonnage_ordered;
			$sheet->deliveredtonnage = $tonnage_delivered;
	...
	/*
	public function getSalesOrders($profile, $customernumber) {
	...
	public function getOrderLines($profile, $customernumber) {
		$dbAsw = $this->load->database('asw', TRUE);
		$dbAsw->where(param('param_asw_database_column_soline_state').' !=', param('param_asw_database_column_soline_state_deleted'));
		$dbAsw->where(param('param_asw_database_column_soline_product'), $profile);
		$dbAsw->where(param('param_asw_database_column_soline_customernumber'), $customernumber);
		$dbAsw->where(param('param_asw_database_column_soline_orderdate').' > ', '20120101');
